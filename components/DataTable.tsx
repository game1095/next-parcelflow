...
type SummaryRow = {
  ...
  total: number;
  no_photo: number;
  no_photo_details: { tiktok: number; shopee: number; lazada: number; other: number };
...
export default function DataTable({ data }: { data: SummaryRow[] }) {
  ...
  const [selectedNoPhotoRow, setSelectedNoPhotoRow] = useState<SummaryRow | null>(null);
  ...
  const groupedNoPhotoItems = useMemo(() => {
    ...
    selectedNoPhotoRow.no_photo_items.forEach(item => {
      const rawDate = item.report_date ? item.report_date.split('T')[0] : "ไม่ระบุวันที่";
    ...
    return Object.keys(groups).sort((a, b) => {
      ...
      date: dateKey === "ไม่ระบุวันที่" ? "ไม่ระบุวันที่" : new Date(dateKey).toLocaleDateString('th-TH', { year: 'numeric', month: 'long', day: 'numeric' }),
    ...
    <div className="w-full">
            ...
            <tr>
              ...
              >
                <div className="flex items-center gap-2">
                  ที่ทำการ (Office) / Postcode
                  <span className="text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300">{getSortIcon("office")}</span>
                </div>
              </th>
              <th 
              ...
              >
                <div className="flex items-center justify-center gap-2">
                  วันที่รายงาน
                  <span className="text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300">{getSortIcon("report_date")}</span>
                </div>
              </th>
              <th 
                className="px-6 py-4 font-medium text-center cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors group"
              ...
              >
                <div className="flex items-center justify-center gap-2">
                  รวมทั้งหมด (Total)
                  <span className="text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300">{getSortIcon("total")}</span>
                </div>
              </th>
              <th 
                className="px-6 py-4 font-medium text-center cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors group"
              ...
              >
                <div className="flex items-center justify-center gap-2">
                  No Photo
                  <span className="text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300">{getSortIcon("no_photo")}</span>
                </div>
              </th>
              <th 
                className="px-6 py-4 font-medium text-center cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors group"
              ...
              >
                <div className="flex items-center justify-center gap-2">
                  Waiting
                  <span className="text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300">{getSortIcon("waiting")}</span>
                </div>
              </th>
              <th 
                className="px-6 py-4 font-medium text-center cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors group"
              ...
              >
                <div className="flex items-center justify-center gap-2">
                  Completed
                  <span className="text-gray-400 group-hover:text-gray-600 dark:group-hover:text-gray-300">{getSortIcon("completed")}</span>
                </div>
              </th>
            </tr>
          </thead>
              ...
              paginatedData.map((row, index) => (
                ...
                >
                  <td className="px-6 py-4">
                    <div className="font-medium text-gray-900 dark:text-gray-100">{row.office}</div>
                    <div className="text-xs text-gray-500 mt-0.5">{row.post_code}</div>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <span className="text-gray-600 dark:text-gray-400">
                      {row.report_date ? new Date(row.report_date).toLocaleDateString('th-TH') : '-'}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <span className="font-bold text-gray-800 dark:text-gray-200">{row.total}</span>
                  </td>
                  <td className="px-6 py-2 text-center">
                    <button 
                      onClick={() => row.no_photo > 0 && setSelectedNoPhotoRow(row)}
                      className={`flex flex-col items-center gap-1.5 w-full p-2 rounded-lg transition-colors ${
                        row.no_photo > 0 ? 'hover:bg-red-50 dark:hover:bg-red-900/20 cursor-pointer' : 'cursor-default'
                      }`}
                    >
                      ...
                      {row.no_photo > 0 && (
                        <div className="flex items-center gap-1.5 text-[10px] mt-0.5">
                          {row.no_photo_details.tiktok > 0 && (
                            <span className="bg-black text-white px-1.5 py-0.5 rounded shadow-sm" title="Tiktok">T: {row.no_photo_details.tiktok}</span>
                          )}
                          {row.no_photo_details.shopee > 0 && (
                            <span className="bg-[#ee4d2d] text-white px-1.5 py-0.5 rounded shadow-sm" title="Shopee">S: {row.no_photo_details.shopee}</span>
                          )}
                          {row.no_photo_details.lazada > 0 && (
                            <span className="bg-[#0f146d] text-white px-1.5 py-0.5 rounded shadow-sm" title="Lazada">L: {row.no_photo_details.lazada}</span>
                          )}
                        </div>
                      )}
                    </button>
      ...
      <div className="p-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 rounded-b-2xl flex flex-col sm:flex-row justify-between items-center gap-4">
        <span className="text-sm text-gray-500 dark:text-gray-400">
          แสดงข้อมูล {filteredAndSortedData.length > 0 ? (currentPage - 1) * pageSize + 1 : 0} ถึง {Math.min(currentPage * pageSize, filteredAndSortedData.length)} จากทั้งหมด {filteredAndSortedData.length} รายการ
        </span>
      ...
      {selectedNoPhotoRow && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm" onClick={() => setSelectedNoPhotoRow(null)}>
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden transform transition-all" onClick={e => e.stopPropagation()}>
            <div className="p-5 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/80 dark:bg-gray-800/80">
              ...
              <button onClick={() => setSelectedNoPhotoRow(null)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700">
                  ...
                  {groupedNoPhotoItems.map((group, groupIdx) => (
                    <div key={groupIdx} className="space-y-3 relative">
                      <div className="sticky top-0 z-10 -mx-4 px-4 py-2 bg-gray-50/95 dark:bg-gray-900/95 backdrop-blur-md border-y border-gray-200 dark:border-gray-700 flex items-center gap-2">
                        ...
                        <h4 className="font-bold text-gray-800 dark:text-gray-200">{group.date}</h4>
                        <span className="text-xs bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded-full">{group.items.length} รายการ</span>
                      </div>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-sm text-gray-600 dark:text-gray-300">
                                <div className="flex items-start gap-1.5">
                                  <span className="text-gray-400 dark:text-gray-500 w-16 flex-shrink-0">ผู้ดูแล:</span> 
                                  <span className="font-medium text-gray-800 dark:text-gray-200 break-words">{item.user_name}</span>
                                </div>
                                <div className="flex items-start gap-1.5">
            ...
            <div className="p-4 border-t border-gray-100 dark:border-gray-700 bg-gray-50/80 dark:bg-gray-800/80 flex justify-end">
              <button onClick={() => setSelectedNoPhotoRow(null)} className="px-5 py-2.5 bg-gray-200 hover:bg-gray-300 text-gray-800 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-200 rounded-xl font-semibold transition-colors">
                ปิดหน้าต่าง
              </button>
            </div>
          </div>
