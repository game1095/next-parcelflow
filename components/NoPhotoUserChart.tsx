...
const tooltipStyle = { borderRadius: '16px', border: 'none', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1), 0 8px 10px -6px rgba(0,0,0,0.1)', backgroundColor: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(8px)', padding: '12px 16px' };
...
export default function NoPhotoUserChart({ items }: { items: NoPhotoItem[] }) {
  ...
  const availableNames = useMemo(() => {
    const nameMap: Record<string, { baseName: string, office: string, count: number }> = {};
    
    items.forEach(item => {
      const name = item.name && item.name.trim() !== "-" ? item.name : "ไม่ระบุชื่อลูกค้า";
  ...
  const chartData = useMemo(() => {
    ...
    if (selectedName !== "all") {
      ...
      filteredItems.forEach(item => {
        const dateKey = item.report_date ? item.report_date.split('T')[0] : "ไม่ระบุวันที่";
      ...
      const dataArray = Object.entries(dateMap).map(([date, count]) => ({
        date,
        count,
        formattedDate: date === "ไม่ระบุวันที่" ? date : new Date(date).getDate().toString()
      }));
      ...
      const dataArray = Object.values(dateMap).map(row => {
        ...
        return {
          ...row,
          formattedDate: row.date === "ไม่ระบุวันที่" ? row.date : new Date(row.date).toLocaleDateString('th-TH', { month: 'short', day: 'numeric' })
        };
      });
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div style={tooltipStyle}>
          <p style={{ color: '#374151', fontWeight: 700, marginBottom: '8px' }}>วันที่: {label}</p>
          <div className="space-y-1">
            {showData.map((entry: any, index: number) => (
              <div key={index} className="flex items-center justify-between gap-4 text-xs">
                <span className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-sm" style={{ backgroundColor: entry.color }} />
                  <span className="text-gray-700 font-medium truncate max-w-[150px]">{entry.name}</span>
                </span>
      ...
      <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="text-base font-bold text-gray-900 dark:text-gray-100">
            แนวโน้มปัญหา No Photo รายลูกค้า (รายวัน) {selectedName === "all" ? "- Top 10" : ""}
          </h2>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1 flex items-center gap-2">
            <span>ติดตามพฤติกรรม No Photo ของลูกค้าแต่ละราย</span>
            {displayMonth && (
        ...
        <div className="relative w-full sm:w-64">
          <select className="block w-full p-2.5 text-xs font-semibold text-gray-700 border border-gray-200 rounded-xl bg-gray-50/80 focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400 dark:bg-gray-800/80 dark:border-gray-600 dark:text-gray-300 appearance-none cursor-pointer transition-all" value={selectedName} onChange={(e) => setSelectedName(e.target.value)}>
            <option value="all">ภาพรวมทุกคน (All Customers)</option>
            {availableNames.map(nameObj => (<option key={nameObj.value} value={nameObj.value}>{nameObj.label}</option>))}
          </select>
      ...
      <div className="p-6">
        {chartData.length > 0 ? (
            ...
            <ResponsiveContainer width="100%" height="100%">
              {selectedName === "all" ? (
                <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                  ...
                  <XAxis dataKey="formattedDate" tick={{ fill: '#9ca3af', fontSize: 11, fontWeight: 500 }} axisLine={false} tickLine={false} dy={10} />
                  <YAxis allowDecimals={false} tick={{ fill: '#9ca3af', fontSize: 11 }} axisLine={false} tickLine={false} dx={-10} />
                  <Tooltip content={renderLineTooltip} />
                  <Legend verticalAlign="top" height={40} wrapperStyle={{ fontSize: '13px', paddingTop: '10px', paddingBottom: '20px' }} />
                ...
                >
                  ...
                  <XAxis dataKey="formattedDate" tick={{ fill: '#9ca3af', fontSize: 11, fontWeight: 500 }} axisLine={false} tickLine={false} dy={10} />
                  <YAxis allowDecimals={false} tick={{ fill: '#9ca3af', fontSize: 11 }} axisLine={false} tickLine={false} dx={-10} />
                  <Tooltip cursor={{ stroke: '#0d9488', strokeWidth: 2, strokeDasharray: '5 5' }} contentStyle={tooltipStyle} itemStyle={{ color: '#0d9488', fontWeight: 700 }} labelStyle={{ color: '#374151', fontWeight: 700, marginBottom: '4px' }} labelFormatter={(label) => `วันที่: ${label}`} formatter={(value: any) => [`${value} ครั้ง`, 'จำนวน No Photo']} />
                  <Area 
                    type="monotone" 
                    dataKey="count" 
                    stroke="#0d9488" 
          ...
          <div className="flex flex-col items-center justify-center h-64 text-gray-500 dark:text-gray-400">
            ...
            <p className="text-lg">ไม่มีข้อมูลในช่วงเวลาหรือลูกค้าที่เลือก</p>
          </div>
        )}
      </div>
    </div>
