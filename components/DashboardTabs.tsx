...
export default function DashboardTabs({ rawData }: DashboardTabsProps) {
  const [activeTab, setActiveTab] = useState<"overview" | "nophoto" | "userchart">("overview");
  ...
  const [globalStartDate, setGlobalStartDate] = useState<string>(getTodayString());
  const [globalEndDate, setGlobalEndDate] = useState<string>(getTodayString());

  const PROVINCE_GROUPS: Record<string, string[]> = useMemo(() => ({
    "ปจ.นครสวรรค์": ["60000", "60001", "60002", "60110", "60120", "60130", "60140", "60150", "60160", "60170", "60180", "60190", "60210", "60220", "60230", "60240", "60250", "60260", "428"],
    "ปจ.อุทัยธานี": ["61000", "61110", "61120", "61130", "61140", "61150", "61160", "61170", "61180"],
    "ปจ.กำแพงเพชร": ["62000", "62110", "62120", "62130", "62140", "62150", "62160", "62170", "62180", "62190", "62210", "89"],
    "ปจ.ตาก": ["63000", "63110", "63111", "63120", "63130", "63140", "63150", "63160", "63170", "63180", "58", "154"],
    "ปจ.สุโขทัย": ["64000", "64110", "64120", "64130", "64140", "64150", "64160", "64170", "64180", "64190", "64210", "64220", "64230"],
    "ปจ.พิษณุโลก": ["65000", "65001", "65110", "65120", "65130", "65140", "65150", "65160", "65170", "65180", "65190", "65210", "65220", "65230", "65240", "36", "61", "112", "287", "303"],
    "ปจ.พิจิตร": ["66000", "66110", "66120", "66130", "66140", "66150", "66160", "66170", "66180", "66190", "66210", "66220", "66230"],
    "ปจ.เพชรบูรณ์": ["67000", "67110", "67120", "67130", "67140", "67150", "67160", "67170", "67180", "67190", "67210", "67220", "67230", "67240", "67250", "67260", "67270", "67280"]
  }), []);

  const { summaryData, noPhotoNameData, noPhotoRawItems, monthNoPhotoItems, kpiData } = useMemo(() => {
    ...
    baseFilteredData.forEach(row => {
      if (row.report_date && row.report_date.startsWith(targetMonth)) {
        ...
        if (status.includes("no photo") || status.includes("nophoto") || status === "no photo" || status === "nophoto") {
          monthItems.push({
            barcode: row.barcode || "-",
            user_name: row.user_name || "-",
            name: row.name && row.name.trim() !== "-" ? row.name : "ไม่ระบุชื่อลูกค้า",
    ...
    filteredData.forEach((row) => {
      ...
      } else if (row.report_date && summaryMap[key].report_date && new Date(row.report_date) > new Date(summaryMap[key].report_date)) {
        summaryMap[key].report_date = row.report_date;
      }
      ...
      if (status.includes("no photo") || status.includes("nophoto") || status === "no photo" || status === "nophoto") {
        ...
        else if (platform === "shopee") summaryMap[key].no_photo_details.shopee++;
        else if (platform === "lazada") summaryMap[key].no_photo_details.lazada++;
    ...
    return {
      summaryData: Object.values(summaryMap).sort((a, b) => b.no_photo - a.no_photo),
      noPhotoNameData: Object.values(noPhotoNameSummary).map((item: any) => ({
        ...item,
  ...
  }, [rawData, globalPlatform, globalProvince, globalStartDate, globalEndDate, PROVINCE_GROUPS]);
    ...
    <div className="space-y-6">
      ...
      <div className="backdrop-blur-xl bg-white/80 dark:bg-[#161a24]/80 p-5 rounded-2xl shadow-sm border border-gray-200/60 dark:border-gray-700/60 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-3 text-gray-800 dark:text-gray-200">
          ...
          <span className="font-bold text-sm tracking-tight">ตัวกรองข้อมูล</span>
        </div>

        <div className="flex flex-col sm:flex-row items-center gap-2.5 w-full md:w-auto">
          <div className="relative w-full sm:w-48">
            <select className="block w-full p-2.5 pl-3 pr-10 text-xs font-semibold text-gray-700 border border-gray-200 rounded-xl bg-gray-50/80 focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 dark:bg-gray-800/80 dark:border-gray-600 dark:text-gray-300 appearance-none cursor-pointer transition-all" value={globalProvince} onChange={(e) => setGlobalProvince(e.target.value)}>
              <option value="all">ทุกพื้นที่ (ปจ.)</option>
              {Object.keys(PROVINCE_GROUPS).map(province => (<option key={province} value={province}>{province}</option>))}
            </select>
            <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-3 text-gray-400"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7"></path></svg></div>
          </div>
          <div className="relative w-full sm:w-40">
            <select className="block w-full p-2.5 pl-3 pr-10 text-xs font-semibold text-gray-700 border border-gray-200 rounded-xl bg-gray-50/80 focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 dark:bg-gray-800/80 dark:border-gray-600 dark:text-gray-300 appearance-none cursor-pointer transition-all" value={globalPlatform} onChange={(e) => setGlobalPlatform(e.target.value)}>
          ...
          <div className="flex items-center gap-1.5 w-full sm:w-auto">
            <input type="date" className="block w-full sm:w-36 p-2.5 text-xs font-semibold text-gray-700 border border-gray-200 rounded-xl bg-gray-50/80 focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 dark:bg-gray-800/80 dark:border-gray-600 dark:text-gray-300 outline-none transition-all" value={globalStartDate} onChange={(e) => setGlobalStartDate(e.target.value)} />
            <span className="text-gray-300 dark:text-gray-600 font-bold text-xs">→</span>
            <input type="date" className="block w-full sm:w-36 p-2.5 text-xs font-semibold text-gray-700 border border-gray-200 rounded-xl bg-gray-50/80 focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 dark:bg-gray-800/80 dark:border-gray-600 dark:text-gray-300 outline-none transition-all" value={globalEndDate} onChange={(e) => setGlobalEndDate(e.target.value)} />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="group relative overflow-hidden bg-white dark:bg-[#161a24] rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-800 flex items-center gap-4 hover:shadow-lg hover:shadow-blue-500/5 hover:-translate-y-0.5 transition-all duration-300">
          ...
          <div>
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">พัสดุทั้งหมด</p>
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{kpiData.totalParcels.toLocaleString()} <span className="text-sm font-normal text-gray-500">ชิ้น</span></h3>
          </div>
        </div>
        ...
        <div className="group relative overflow-hidden bg-white dark:bg-[#161a24] rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-800 flex items-center gap-4 hover:shadow-lg hover:shadow-red-500/5 hover:-translate-y-0.5 transition-all duration-300">
          ...
          <div>
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">ลืมถ่ายรูป (No Photo)</p>
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{kpiData.totalNoPhoto.toLocaleString()} <span className="text-sm font-normal text-gray-500">ครั้ง</span></h3>
          </div>
        </div>
        ...
        <div className="group relative overflow-hidden bg-white dark:bg-[#161a24] rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-800 flex items-center gap-4 hover:shadow-lg hover:shadow-orange-500/5 hover:-translate-y-0.5 transition-all duration-300">
          ...
          <div>
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">อัตราส่วนความผิดพลาด</p>
            <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{kpiData.errorRate}% <span className="text-sm font-normal text-gray-500">ของทั้งหมด</span></h3>
          </div>
        </div>
        ...
        <div className="group relative overflow-hidden bg-white dark:bg-[#161a24] rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-gray-800 flex items-center gap-4 hover:shadow-lg hover:shadow-purple-500/5 hover:-translate-y-0.5 transition-all duration-300">
          ...
          <div className="overflow-hidden">
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">สาขาที่ปัญหาเยอะสุด</p>
            <h3 className="text-lg font-bold text-gray-900 dark:text-white truncate" title={kpiData.mostProblematicOffice}>{kpiData.mostProblematicOffice}</h3>
            {kpiData.maxOfficeCount > 0 && <p className="text-xs text-red-500 font-medium">พลาด {kpiData.maxOfficeCount.toLocaleString()} ครั้ง</p>}
          </div>
        </div>
      </div>
